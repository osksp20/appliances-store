...
public class ClientesService extends GenericService<Cliente> {
	...
	protected void validaElementoAModificar(Cliente cliente) throws Exception {
		...
		if(cliente.getDireccion() == null) {
			throw new Exception("La dirección del cliente a modificar no puede ser nula");
		}
		if(cliente.getTelefono() == null) {
			throw new Exception("El teléfono del cliente a modificar no puede ser nulo");
		}
		if(cliente.getUltimaNomina() != null) {
			...
			if(comprobarAnterior) {
				...
				if(cal.compareTo(nominaCal) > 0) {
					throw new Exception("La fecha de la última nómina no puede ser anterior a un mes de la fecha actual");
				}
			}
		}
	}
